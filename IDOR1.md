...
Натискаємо **View Profile** і бачимо, що на сторінці відображаються лише `name`, `color` та `size`. Порівнюємо це з переліком атрибутів, яких немає на екрані, та підтверджуємо відповідь — сервер повертає ще й `role` та `userId`:

![Перегляд профілю у WebGoat: name/color/size на екрані, role та userId — приховані](screenshots/webgoat-view-profile.png)

Переглядаємо HTTP-історію у Burp Suite та знаходимо ці відмінності у «сирій» відповіді сервера — окрім `name`, `color` та `size`, у JSON присутні ще й `role` та `userId`:

![Burp Suite: сира відповідь /WebGoat/IDOR/profile з полями role та userId](screenshots/burp-raw-response.png)

Як результат — цей крок пройдено (правильна відповідь `role, userId` зафіксована на скріншоті вище).
...
Перевіряємо здогадку в Burp: запит `GET /WebGoat/IDOR/profile` (без ідентифікатора) повертає порожній об'єкт `{}` — сервер справді не розуміє, чий профіль треба віддати, поки шлях не містить конкретного ID:
...
Для цього потрібно знайти відповідний запит та модифікувати його в "Intruder":

![step06](https://github.com/dimdimuzun/CS-practice/raw/main/InsecureDirectObjectReference/idor06.png)

Результат запуску атаки "intruder" дасть схожі результати:
...
Для вирішення цього питання потрібно мати актуальний (час обмежений) **JSESSIONID**. Далі з "свіжого" успішного запиту **GET** зробити в **Repeater** запит **PUT**. Детально запит представлений на наступному скріншоті.

![step08](https://github.com/dimdimuzun/CS-practice/raw/main/InsecureDirectObjectReference/idor08.png)

Виходячи з відповіді все зроблено вірно, але запропоновано ще раз зменшити роль (2->1), тому відповідно вносимо зміни та отримуємо результат:
